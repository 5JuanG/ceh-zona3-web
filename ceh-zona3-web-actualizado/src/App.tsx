...
function MainLayout({ currentUser, onLogout }: MainLayoutProps) {
  ...
  const [providerToEdit, setProviderToEdit] = useState<Doctor | null>(null);

  const [adminStaffModalOpen, setAdminStaffModalOpen] = useState(false);
  const [adminStaffToEdit, setAdminStaffToEdit] = useState<Doctor | null>(null);
  ...
  const handleOpenProviderModal = (doc?: Doctor) => {
    setProviderToEdit(doc || null);
    setProviderModalOpen(true);
  };

  const handleOpenAdminStaffModal = (doc?: Doctor) => {
    setAdminStaffToEdit(doc || null);
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans text-slate-900 flex flex-col antialiased">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {activeTab === 'doctors' && (
          <DoctorDirectory
            ...
            onOpenAdminStaffModal={handleOpenAdminStaffModal}
      ...
      <AdminStaffModal
        isOpen={adminStaffModalOpen}
        onClose={() => setAdminStaffModalOpen(false)}
        staffToEdit={adminStaffToEdit}
      />
