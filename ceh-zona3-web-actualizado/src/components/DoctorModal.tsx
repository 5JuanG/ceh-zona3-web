...
export const DoctorModal: React.FC<DoctorModalProps> = ({ isOpen, onClose, doctorToEdit }) => {
  ...
  const getCombinedDoctorData = (): Partial<Doctor> => {
    ...
    // Los médicos colaboradores/consultores nuevos quedan 'pendiente' hasta
    // que un administrador los autorice desde el Panel de Admin. Al editar,
    // se respeta el estatus que ya tenía. Proveedores de salud y contactos
    // administrativos no pasan por este flujo de aprobación.
    const needsApprovalFlow = type === 'colaborador' || type === 'consultor';
    const approvalStatus = doctorToEdit
      ? (doctorToEdit.approvalStatus || (needsApprovalFlow ? 'pendiente' : 'autorizado'))
      : (needsApprovalFlow ? 'pendiente' : 'autorizado');
