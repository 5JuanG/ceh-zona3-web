    ...
    <div className="space-y-6 pb-12">
      ...
      {activeSubTab === 'congregations' && (
        <div className="space-y-4">
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-2xl shadow-sm border p-4 space-y-3 ${
                    cong.isExcludedFromTerritory ? 'border-amber-300 bg-amber-50/50' : 'border-slate-200'
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="text-[11px] font-bold text-slate-400">#{cong.number}</span>
                      <h4 className="font-bold text-slate-900 text-sm leading-tight">{cong.name}</h4>
                      <p className="text-xs text-slate-500 mt-0.5">{cong.city}</p>
                    </div>
                    <button
                      onClick={() => {
                      ...
                      className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-200 shrink-0"
                  ...
                  <div className="flex flex-wrap items-center gap-1.5 text-[11px]">
                    ...
                    <span className="text-slate-500">{cong.language}</span>
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 mb-1 uppercase tracking-wide">
                      Miembro Responsable CEH
                    </label>
                    {cong.isExcludedFromTerritory ? (
                      <span className="text-slate-400 italic text-[11px]">No requiere asignación</span>
                    ) : (
                      <div className="flex items-center gap-2">
                        {assignedMember && (
                          <span
                        ...
                        <select
                          ...
                          className={`w-full px-2.5 py-2 rounded-lg border text-xs font-semibold ${
                            assignedMember
                  ...
                  <button
                    ...
                    className={`w-full px-2.5 py-2 rounded-lg text-xs font-bold border transition-colors ${
                      cong.isExcludedFromTerritory
                        ? 'bg-amber-100 text-amber-800 border-amber-300 hover:bg-amber-200'
                        : 'bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200'
                    }`}
                  >
                    {cong.isExcludedFromTerritory ? 'Excluida del reparto (Reactivar)' : 'Excluir del reparto territorial'}
                  </button>
                </div>
              );
            })}
          ...
          {/* Table of Congregations (Desktop) */}
