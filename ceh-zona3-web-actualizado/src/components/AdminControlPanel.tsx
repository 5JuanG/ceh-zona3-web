...
export const AdminControlPanel: React.FC<AdminControlPanelProps> = ({
  ...
  const pendingDoctors = doctors.filter(d => d.approvalStatus === 'pendiente');
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-sky-950 text-white rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-700/80 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          ...
          {pendingDoctors.length > 0 && (
            <div className="bg-amber-900/30 border border-amber-500/40 rounded-2xl p-4 space-y-3 text-xs">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-amber-300 shrink-0" />
                <h4 className="font-bold text-amber-200">
                  Médicos Pendientes de Aprobación ({pendingDoctors.length})
                </h4>
              </div>
              <p className="text-slate-300 text-[11px]">
                Estos contactos se registraron desde el formulario pero todavía no han sido autorizados por la sucursal. No aparecen en el Directorio de Médicos hasta que los apruebes aquí.
              </p>
              <div className="space-y-2">
                {pendingDoctors.map(doc => (
                  <div
                    key={doc.id}
                    className="bg-slate-800/60 border border-slate-700/60 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                  >
                    <div>
                      <span className="font-bold text-white">{doc.title} {doc.name}</span>
                      <span className="text-slate-400 ml-2">
                        {doc.type === 'colaborador' ? 'Médico Colaborador' : 'Médico Consultor'} · {doc.specialty}
                      </span>
                      {doc.phoneMobile && <span className="text-slate-400 ml-2">• {doc.phoneMobile}</span>}
                      {doc.email && <span className="text-slate-400 ml-2">• {doc.email}</span>}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <button
                        onClick={() => {
                          updateDoctor(doc.id, { approvalStatus: 'autorizado' });
                          showNotification(`${doc.name} fue autorizado y ahora aparece en el Directorio de Médicos.`);
                        }}
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg shadow flex items-center gap-1.5"
                      >
