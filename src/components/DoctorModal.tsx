...
import { resolveAndParseGoogleMapsUrl, sanitizeHospitalCoordinates, parseLatAndLng } from '../utils/googleMapsParser';
import { 
  X, 
  UserPlus, 
  Save, 
...
export const DoctorModal: React.FC<DoctorModalProps> = ({ isOpen, onClose, doctorToEdit }) => {
  ...
  // Ubicación (solo relevante para 'proveedor_salud' / 'contacto_administrativo')
  ...
  useEffect(() => {
    if (doctorToEdit) {
      ...
      setGoogleMapsUrl(doctorToEdit.googleMapsUrl || '');
      setCoordinates(doctorToEdit.coordinates);
      setLatInput(doctorToEdit.coordinates ? String(doctorToEdit.coordinates.lat) : '');
      setLngInput(doctorToEdit.coordinates ? String(doctorToEdit.coordinates.lng) : '');
      setParseStatusMsg('');
    ...
    // NOTA: 'hospitals' y 'cehMembers' se usan solo para calcular valores
    // por defecto al abrir el modal. NO deben ir en este arreglo de
    // dependencias: como vienen de un listener en tiempo real de Firestore,
    // se recrean (nueva referencia) cada vez que llega una actualización de
    // la nube -aunque el contenido no cambie-, lo que reiniciaba todo el
    // formulario a mitad de la captura. Solo debe reiniciarse al abrir el
    // modal o al cambiar el médico que se está editando.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [doctorToEdit, isOpen]);

  if (!isOpen) return null;

  // Extrae coordenadas GPS de un link de Google Maps para ubicar el
  // consultorio/oficina de un Proveedor de Salud o Colaborador
  // Administrativo en el Mapa Interactivo (igual que con los hospitales).
  const handleParseGoogleMapsLink = async (urlToParse?: string) => {
    ...
    try {
      ...
      if (parsed.coordinates) {
        ...
        setParseStatusMsg(`📍 ¡Coordenadas GPS (${parsed.coordinates.lat.toFixed(6)}, ${parsed.coordinates.lng.toFixed(6)}) extraídas del enlace!`);
      } else {
        setParseStatusMsg('ℹ️ No se detectaron coordenadas en el enlace.');
      }
      if (parsed.extractedAddress && !address) {
        setAddress(parsed.extractedAddress);
      }
    } catch (e) {
      setParseStatusMsg('⚠️ No se pudo extraer la coordenada del enlace.');
    } finally {
  ...
  const getCombinedDoctorData = (): Partial<Doctor> => {
    ...
    return {
      ...
      coordinates: sanitizeHospitalCoordinates(parseLatAndLng(latInput, lngInput) || coordinates)
    };
  };
                    ...
                    {(type === 'proveedor_salud' || type === 'contacto_administrativo') && (
                      <div className="mt-3 bg-white border border-sky-300 rounded-xl p-3">
                        <label className="block font-bold text-slate-800 mb-1 flex items-center gap-1.5">
                          ...
                          Ubicación (para mostrarlo en el Mapa Interactivo)
                        </label>
                        <div className="flex gap-2">
                          <input
                            ...
                            placeholder="Pega aquí el link de Google Maps del consultorio/oficina"
                          ...
                          <button
                            ...
                            className="px-3 py-2 bg-sky-600 text-white rounded-lg text-sm font-bold disabled:opacity-50"
                          >
                            {isParsingUrl ? 'Buscando...' : 'Buscar'}
                          </button>
                        </div>
                        {parseStatusMsg && (
                          <p className="text-[11px] text-slate-600 mt-1.5">{parseStatusMsg}</p>
                        )}
                        {coordinates && (
                          <p className="text-[11px] text-emerald-700 font-semibold mt-1">
                            ✓ Ubicación guardada ({coordinates.lat.toFixed(5)}, {coordinates.lng.toFixed(5)})
                          </p>
                        )}
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div>
                            <label className="block text-[10px] font-bold text-slate-600 mb-0.5">Latitud GPS (si el link no funcionó)</label>
                            <input
                              type="text"
                        ...
                        <p className="text-[10px] text-slate-500 mt-1">
                          Tip: si el link no funciona, ábrelo en tu navegador, espera a que cargue el mapa, y copia los dos números que aparecen en la barra de direcciones después de "@" (ej. https://www.google.com/maps/@<strong>25.6880,-100.3120</strong>,17z) — pégalos aquí arriba.
                        </p>
                      </div>
                    )}
                  </div>
