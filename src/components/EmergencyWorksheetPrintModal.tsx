...
export const EmergencyWorksheetPrintModal: React.FC<EmergencyWorksheetPrintModalProps> = ({
  ...
  // Genera el PDF directamente (html2canvas-pro + jsPDF), igual que en la
  // Hoja de Trabajo de Integrantes CEH. Esto evita por completo el
  // encabezado/pie de página que el propio navegador agrega cuando se usa
  // window.print() -> "Guardar como PDF" (fecha, título de la pestaña y
  // URL), que era lo que empujaba el contenido y provocaba saltos de
  // apartados entre páginas.
  const handleDescargarPDF = async () => {
    if (generandoPDF) return;
    const pagina1 = document.getElementById('hoja-emergencia-pagina-1');
    const pagina2 = document.getElementById('hoja-emergencia-pagina-2');
    ...
    try {
      ...
      console.error('Error generando el PDF de la Hoja de Emergencia:', err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-start bg-slate-900/80 backdrop-blur-sm overflow-y-auto overflow-x-hidden p-2 sm:p-6 print:p-0 print:bg-white print:static print:overflow-visible">
      ...
      <div className="w-full max-w-4xl bg-slate-900 text-white rounded-t-xl p-4 flex items-center justify-between shadow-lg border-b border-slate-700 print:hidden sticky top-2 z-20">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-60 disabled:cursor-wait text-slate-950 font-bold text-xs rounded-lg shadow transition-colors"
          >
          ...
          <button
            onClick={handlePrint}
            className="inline-flex items-center gap-1.5 px-3 py-2 bg-slate-800 hover:bg-slate-700 text-white font-semibold text-xs rounded-lg shadow transition-colors border border-slate-700"
            title="Imprimir directo con la impresora del sistema (no recomendado para guardar el PDF, ya que el navegador puede agregar su propio encabezado/pie de página)"
          >
      ...
      <div id="printable-worksheet" className="w-full max-w-[800px] bg-white text-black p-6 sm:p-8 rounded-b-xl shadow-2xl border border-slate-300 font-sans text-[11px] leading-tight space-y-4 print:shadow-none print:border-none print:p-0 print:m-0 print:w-full print:max-w-none print:rounded-none">
        ...
        {/* ================= PAGE 1 ================= */}
        <div id="hoja-emergencia-pagina-1" className="space-y-2 border border-slate-800 p-1 flex flex-col justify-between min-h-[950px] bg-white">
        ...
        {/* ================= PAGE 2 ================= */}
        <div id="hoja-emergencia-pagina-2" className="page-break space-y-3 border border-slate-800 p-1 pt-2 flex flex-col justify-between min-h-[950px] bg-white">
