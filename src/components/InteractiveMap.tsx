...
interface InteractiveMapProps {
  ...
  /** Abre la ficha de un Proveedor de Salud / Colaborador Administrativo
   * al hacer clic en su pin. Opcional: si no se pasa, esos pines no
   * responden al clic (solo se ven, útil en vistas de solo lectura). */
  onOpenDoctorModal?: (doc?: any) => void;
}
...
// Círculo pequeño con el color del miembro CEH responsable del hospital.
// Sustituye el pin rojo genérico cuando ya sabemos a quién pertenece.
const crearIconoHospitalPorMiembro = (color: string) => L.divIcon({
...
// Cuadrado pequeño (para diferenciar del círculo de hospitales) con el
// color del miembro CEH, usado para Proveedores de Salud y Colaboradores
// Administrativos.
const crearIconoContactoPorMiembro = (color: string) => L.divIcon({
  html: `<div style="background:${color};width:15px;height:15px;border-radius:4px;border:2px solid white;box-shadow:0 1px 4px rgba(0,0,0,0.5);"></div>`,
  className: '',
  iconSize: [15, 15],
  iconAnchor: [7, 7],
  popupAnchor: [0, -7]
...
export const InteractiveMap: React.FC<InteractiveMapPropsExtended> = ({
  ...
  const { hospitals, doctors, cehMembers: globalMembers, congregations: allCongregations, updateHospital } = useApp();
  ...
  const contactosLayerRef = useRef<L.LayerGroup | null>(null);
  ...
  // Filtro de capas del mapa: qué tipo de pin mostrar.
  const [mostrarHospitales, setMostrarHospitales] = useState(true);
  const [mostrarProveedores, setMostrarProveedores] = useState(true);
  const [mostrarAdministrativos, setMostrarAdministrativos] = useState(true);
  ...
  useEffect(() => {
    ...
    contactosLayerRef.current = L.layerGroup().addTo(map);
  ...
  useEffect(() => {
    ...
    if (hospitals && hospitals.length > 0 && mostrarHospitales) {
      hospitals.forEach(h => {
        ...
        // Color del círculo = color del miembro CEH responsable de este
        // hospital. Si no tiene miembro asignado, usa el pin rojo genérico.
        const miembroResponsable = membersList?.find(m => m.id === h.assignedCEHMemberId);
        const icon = miembroResponsable
          ? crearIconoHospitalPorMiembro(miembroResponsable.color)
          : hospitalDivIcon;
        ...
        // Hover (tooltip): solo datos generales, sin bloquear el mapa.
        const nombreResponsable = miembroResponsable?.name;
        marker.bindTooltip(
          `<strong>${h.name}</strong><br/>Urgencias: ${h.phoneEmergency || 'N/A'}${nombreResponsable ? `<br/>Responsable: ${nombreResponsable}` : ''}`,
        ...
        if (canEdit) {
          ...
          // Clic: abre el formulario completo de edición.
  ...
  }, [hospitals, effectiveFilterEmail, membersList, pickerMode, readOnly, mostrarHospitales]);

  // Pines de Proveedores de Salud y Colaboradores Administrativos que
  // tengan ubicación guardada (link de Google Maps en su ficha). Se
  // dibujan como cuadrados del color de su miembro CEH responsable, y
  // respetan el mismo filtro de capas del mapa.
  useEffect(() => {
    const map = mapInstanceRef.current;
    if (!map || !contactosLayerRef.current || pickerMode) return;

    contactosLayerRef.current.clearLayers();

    if (!doctors || doctors.length === 0) return;

    doctors.forEach(d => {
      const esProveedor = d.type === 'proveedor_salud';
      const esAdministrativo = d.type === 'contacto_administrativo';
      if (!esProveedor && !esAdministrativo) return;
      if (esProveedor && !mostrarProveedores) return;
      if (esAdministrativo && !mostrarAdministrativos) return;

      const lat = d.coordinates?.lat;
      const lng = d.coordinates?.lng;
      if (lat === undefined || lng === undefined || lat === null || lng === null) return;

      const miembroResponsable = membersList?.find(m => m.id === d.assignedCEHMemberId);
      const color = miembroResponsable?.color || '#64748b';
      const marker = L.marker([lat, lng], { icon: crearIconoContactoPorMiembro(color) });

      const etiquetaTipo = esProveedor ? 'Proveedor de Salud' : 'Colaborador Administrativo';
      marker.bindTooltip(
        `<strong>${d.name}</strong><br/>${etiquetaTipo}${miembroResponsable ? `<br/>Responsable: ${miembroResponsable.name}` : ''}`,
        { direction: 'top', offset: [0, -8], sticky: false }
      );

      if (!readOnly) {
        marker.on('click', () => onOpenDoctorModal?.(d));
      }

      contactosLayerRef.current?.addLayer(marker);
    });
  }, [doctors, membersList, pickerMode, readOnly, mostrarProveedores, mostrarAdministrativos]);
    ...
    <div className="w-full h-full flex flex-col gap-3">
      ...
      {expanded && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 px-1 pb-2 text-xs">
          <span className="font-bold text-slate-500 uppercase tracking-wide text-[10px]">Mostrar en el mapa:</span>
          <label className="flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer">
            <input type="checkbox" checked={mostrarHospitales} onChange={e => setMostrarHospitales(e.target.checked)} />
            🏥 Hospitales
          </label>
          <label className="flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer">
            <input type="checkbox" checked={mostrarProveedores} onChange={e => setMostrarProveedores(e.target.checked)} />
            <span className="inline-block w-2.5 h-2.5 rounded-sm bg-slate-500"></span> Proveedores de Salud
          </label>
          <label className="flex items-center gap-1.5 font-semibold text-slate-700 cursor-pointer">
            <input type="checkbox" checked={mostrarAdministrativos} onChange={e => setMostrarAdministrativos(e.target.checked)} />
            <span className="inline-block w-2.5 h-2.5 rounded-sm bg-slate-500"></span> Colaboradores Administrativos
          </label>
        </div>
      )}
