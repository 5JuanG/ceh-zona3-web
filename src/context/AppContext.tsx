...
import { db, defaultDb, authReady, auth } from '../lib/firebase';
...
interface AppContextType {
  ...
  // true si la cuenta con sesión iniciada tiene rol "admin" en Firestore
  // (colección /roles/{uid}). Los datos de solo-lectura para no-admins ya
  // están garantizados por firestore.rules; esta bandera es para que la
  // interfaz oculte/deshabilite botones de edición y evite intentos de
  // guardado que Firestore rechazaría de todas formas.
  isAdmin: boolean;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Al iniciar/cerrar sesión, consulta /roles/{uid} para saber si esta
  // cuenta es admin. Si el documento no existe (miembro normal, sin rol
  // asignado a mano en la consola de Firebase), se queda como false.
  useEffect(() => {
  ...
  // --- Documentos con acceso de escritura restringido a rol "admin" ---
  // Datos operativos (hospitales, médicos, visitas, casos, hojas de
  // trabajo) los puede editar cualquier miembro con cuenta real; solo la
  // gestión de cuentas del comité y la asignación de territorios queda
  // reservada a administradores.
  const DOCUMENTOS_SOLO_ADMIN = ['cehMembers', 'congregations'];

  // Helper to save a document to Firebase Firestore
  const saveDocToCloud = (docName: string, items: any) => {
    if (DOCUMENTOS_SOLO_ADMIN.includes(docName) && !isAdmin) {
      console.warn(`Guardado bloqueado: se requiere rol "admin" para modificar "${docName}".`);
      return;
    }
    try {
  ...
  const syncAllToCloud = async (): Promise<{ ok: boolean; error?: string }> => {
    ...
    ].filter(p => isAdmin || !DOCUMENTOS_SOLO_ADMIN.includes(p.name));
